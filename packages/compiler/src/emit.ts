...
// are bundled and wrapped for the runtime asset, which mounts one setup
...
export function checkEmitPreconditions(
	...
	for (const script of scripts) {
		...
		if (!hasDefaultExport(parsed.program)) {
...
export function emitScriptFiles(
	...
	if (componentScript.issues.some((issue) => issue.severity === "error")) {
		return { files: [], issues: componentScript.issues };
	}
	return {
		files: [
...
function emitComponentScript(
	...
	const registrations = scripts.map((script, index) => {
		...
		const entryFile =
			script.bodySpan && script.bodySpan.file !== componentFile
				...
				: `${componentDir ? `${componentDir}/` : ""}inline-${index + 1}.js`;
	...
	return {
		contents: [
			...
			...registrations
				.filter(
					(registration): registration is string => registration !== undefined,
				)
				.map((registration) => indent(registration, "  ")),
			"})();",
			"",
		].join("\n"),
		issues,
