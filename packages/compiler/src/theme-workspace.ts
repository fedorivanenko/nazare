			...
			pushUniqueDiagnostics(allIssues, [
				{
					...
					message: `Output ${outputPath} has multiple owners: ${existing.owner} and ${artifact.path}`,
					phase: "emit",
				},
			]);
		}
...
function artifactCompletenessIssues(
	...
	return expectedPaths.flatMap((path): Diagnostic[] => {
