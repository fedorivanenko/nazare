...
test("graph store applies stable-ID records transactionally", () => {
	...
	assert.strictEqual(validationStore.getGraph(), previousGraph);
	const conflictingNode = structuredClone(first);
	conflictingNode.nodes.push({
		...structuredClone(conflictingNode.nodes[0]),
		path: "conflicting/path",
	});
	assert.throws(
		() => validationStore.applyGraph(conflictingNode),
		/Conflicting graph node id/,
	);
	assert.strictEqual(validationStore.getGraph(), previousGraph);
});
...
test("workspace rejects duplicate output ownership even for identical contents", () => {
	const result = buildNazareThemeWorkspace([
		{ path: "components/a/card.nz.liquid", contents: "<div>Same</div>" },
		{ path: "components/b/card.nz.liquid", contents: "<div>Same</div>" },
	]);
	assert.equal(hasIssue(result, "THEME_OUTPUT_COLLISION"), true);
	assert.deepEqual(result.emitted.files, []);
});
