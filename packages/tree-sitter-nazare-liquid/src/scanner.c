...
static bool scan_embedded_closing_tag(TSLexer *lexer, const char *name) {
    ...
    if (!scan_str(lexer, name)) return false;
    while (iswspace(lexer->lookahead)) advance(lexer);
    if (lexer->lookahead == '-') advance(lexer);
    if (lexer->lookahead != '%') return false;
    advance(lexer);
...
static bool may_start_regex(int32_t previous) {
    if (previous == 0) return true;
    const char *prefix = "([{=,:;!&|?+-*~^<>";
    for (int i = 0; prefix[i] != '\0'; i++) {
        if (previous == prefix[i]) return true;
    }
    return false;
}

static bool scan_embedded_content(
    TSLexer *lexer,
    enum TokenType token,
    const char *closing_name,
    bool script
) {
    enum { CODE, SINGLE, DOUBLE, TEMPLATE, LINE_COMMENT, BLOCK_COMMENT, REGEX } state = CODE;
    bool escaped = false;
    ...
    while (lexer->lookahead != 0) {
        ...
        if (state == CODE) {
            if (character == '{') {
                ...
                if (scan_embedded_closing_tag(lexer, closing_name)) {
            ...
            if (character == '\'' || character == '"' || (script && character == '`')) {
            ...
            if (character == '/') {
                ...
                if (lexer->lookahead == '*') {
                    advance(lexer);
                    state = BLOCK_COMMENT;
                } else if (script && lexer->lookahead == '/') {
                    advance(lexer);
                    state = LINE_COMMENT;
                } else if (script && may_start_regex(previous)) {
                    state = REGEX;
                    regex_class = false;
                } else {
                    previous = '/';
            ...
            advance(lexer);
            has_content = true;
            if (!iswspace(character)) previous = character;
            lexer->mark_end(lexer);
            continue;
        }
        ...
        if (state == REGEX) {
            if (character == '[') regex_class = true;
            else if (character == ']') regex_class = false;
            else if (character == '/' && !regex_class) {
                state = CODE;
                previous = '/';
            }
            continue;
...
bool tree_sitter_nazare_liquid_external_scanner_scan(
    void *payload,
    TSLexer *lexer,
    ...
    if (valid_symbols[NAZARE_SCRIPT_CONTENT]) {
        return scan_embedded_content(
            lexer,
            NAZARE_SCRIPT_CONTENT,
            "endscript",
            true
    ...
    if (valid_symbols[NAZARE_STYLESHEET_CONTENT]) {
        return scan_embedded_content(
            lexer,
            NAZARE_STYLESHEET_CONTENT,
            "endstylesheet",
...
void *tree_sitter_nazare_liquid_external_scanner_create() { return NULL; }
void tree_sitter_nazare_liquid_external_scanner_destroy(void *payload) {}
unsigned tree_sitter_nazare_liquid_external_scanner_serialize(void *payload, char *buffer) { return 0; }
void tree_sitter_nazare_liquid_external_scanner_deserialize(void *payload, const char *buffer, unsigned length) {}
