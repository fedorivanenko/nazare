...
`dir` defaults to `build.sourceRoot` in `nazare.theme.json`. Theme intelligence analyzes Nazare Liquid, plain Liquid, Shopify JSON, CSS, and JavaScript. `inspect serve` watches accepted project and metadata files automatically. Its public MCP surface has three tools: `find` resolves stable entities, `traverse` follows typed and evidenced relations, and `evidence` fetches source proof on demand. Structured outputs carry `contractVersion: 2` plus analysis revision and completeness. Queries default to 50 records, accept `limit` (maximum 200) and `cursor`, and reject oversized results instead of flooding the transport.
