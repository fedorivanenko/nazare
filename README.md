...
`dir` defaults to `build.sourceRoot` in `nazare.theme.json`. Theme intelligence analyzes Nazare Liquid, plain Liquid, Shopify JSON, CSS/SCSS, and JavaScript. `inspect serve` watches accepted project and metadata files automatically. Its public MCP surface is one `inspect` tool that accepts a semantic subject (project, file, behavior, metafield, symbol, or diagnostic) and a question (`summary`, `dependencies`, `dependents`, `usages`, `impact`, or `diagnostics`). Model-visible JSON carries `contractVersion: 3`, analysis revision, scoped completeness, compact domain answers, and source evidence as path plus 1-based line/character ranges. Queries default to 10 grouped results, accept `limit` (maximum 50) and `cursor`, and never expose graph or evidence IDs.
