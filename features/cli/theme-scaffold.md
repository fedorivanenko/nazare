...
invariants:
  ...
  - config/settings_schema.json is generated by the Vite plugin and must not be scaffold source
  - config/theme.settings.json is the hand-authored base for theme-level settings and is scaffold source
...
codebaseOwnership:
  ...
  mustNotModify:
    ...
    - theme/default/config/settings_schema.json (generated by Vite plugin)
...
- `config/theme.settings.json`: hand-authored base for theme-level settings groups (colors, typography, etc.). Read by the Vite plugin and merged with component `*.settings.json` files to generate `config/settings_schema.json`. May be an empty array `[]` in the minimal scaffold.
...
- `config/theme.settings.json` is present as scaffold source; `config/settings_schema.json` is absent from scaffold source (generated).
...
- [ ] `config/theme.settings.json` is present as scaffold source
  - Verify file exists and is listed in `theme.files`.
- [ ] `config/settings_schema.json` is absent from scaffold source and `theme.files`
  - Verify it is not listed in `theme.files`.
